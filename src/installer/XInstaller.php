 ...
 * @since          1.5
 */
class XInstaller extends JInstaller
{
	...
	public function setAdapter($name, &$adapter = null, $options = Array())
	{
		...
		$class    = "XInstaller" . ucfirst($name);
		if (!class_exists($class)) {
			if (!is_object($adapter)) {
				foreach ($adapters as $x_adapter) {
					// Try to load the adapter object
					$fullpath = dirname(__FILE__) . '/' . 'adapters' . '/' . strtolower($x_adapter) . '.php';
